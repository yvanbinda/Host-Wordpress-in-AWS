...
_-	Install Apache and Related Tools: This command installs Apache, the HTTP server software, along with related tools and the mod_ssl module for SSL support.

-	Enable Apache to Start on Boot: systemctl enable ensures that Apache starts automatically when the system boots up.
-	
-	Start the Apache Service: systemctl start initiates the Apache service._
...
_Enable PHP 7.4 Repository: amazon-linux-extras enable enables the PHP 7.4 repository, allowing you to install this specific version.
...
-	Install Additional PHP Modules: yum install php-{cgi,curl,mbstring,gd,mysqlnd,gettext,json,xml,fpm,intl,zip} -y installs additional PHP modules required for WordPress and its dependencies._
...
_-	Install MySQL 5.7 Repository: This command installs the MySQL 5.7 repository, allowing you to install the specific version of MySQL.
...
_Change ownership for the /var/www/html directory **sudo chown apache:apache -R /var/www/html** 
...
Change Ownership for the /var/www/html Directory: chown apache:apache -R /var/www/html specifically adjusts ownership for the HTML directory, ensuring Apache has the necessary permissions to serve content._
